...
def exibirVelocidadesMarmousi2(): #{
    ## Parte 4
    ## Gerar o perfil (ou perfis) de velocidade a partir do Marmousi2
...
def selecionarTracosDensidadeExemplo(): #{
    marm = np.load(u'data/marm2_density_a66d829606148832997d10a966180133.npy')
    escalax = 1.25
    escalaz = 1.25    
    minx = int(4000. / escalax)
...
def exibirTracosSelecionados(): #{
    selectedTraces = selecionarTracosExemplo()[::10,:]
    ...
    plt.colorbar()
    plt.show(imgplot4)
    selectedTracesD = selecionarTracosDensidadeExemplo()[::10,:]
    print "Tracos selecionados de densidade: ", selectedTraces.shape
