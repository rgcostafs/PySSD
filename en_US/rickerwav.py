...
def testeRicker01(): #{
    ...
    fs = 1000.0 # frequency de amostragem: 1000Hz, uma amostra a cada 1ms
    samples = int(fs*duration) # numero de amostras = 100 amostras
    t = np.linspace(-0.5, 0.5, samples) # tempo
    angle = -30.0 # 30 graus
    frequencyDominante = 30.0 # 30Hz eh a frequency dominante da Ricker
    wavelet0 = rickerShift(t[350:450], frequencyDominante, angle * math.pi / 180.0)
    plt.figure(3)
    plt.plot(t[350:450], wavelet0, 'b')
    plt.show()
#}
...
#    angle = -30.0 # 30 graus
#    frequencyDominante = 30.0 # 30Hz eh a frequency dominante da Ricker
#    return rickerShift(t,frequencyDominante,angle*math.pi/180.0)
#}

def generateRickerWang16(angle = -30.0, frequencyDominante = 30.0): #{
    ...
    rs = rickerShift(t,frequencyDominante,-angle*math.pi/180.0)
...
def showRickerWang16_2(angle = -30, frequencyDominante = 30.0): #{
    plt.figure(2)
    plt.plot(generateRickerWang16(angle, frequencyDominante),'r', linewidth=3.0)
    plt.show()
#}
...
if __name__ == "__main__":
    #testeRicker01()
    if (len(sys.argv) >= 3):
        showRickerWang16_2(float(sys.argv[1]), float(sys.argv[2]))
    else:
        for frequency in range(10,50,10):
            for angle in range(-90,90,15):
                print "Showing ricker with angle %d and dom. freq. %d\n" % (angle, frequency)
                showRickerWang16_2(float(angle), float(frequency))
